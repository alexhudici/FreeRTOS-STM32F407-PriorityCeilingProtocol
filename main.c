...
//Starting the tasks at idle priority + 1 since otherwise they will be competing with
//the idle task (which runs at tskIDLE_PRIORITY) for the processor
#define tLOW ( tskIDLE_PRIORITY + 1UL)
#define tMED ( tskIDLE_PRIORITY + 2UL )
#define tHIGH ( tskIDLE_PRIORITY + 3UL )
#define tHIGHER (tskIDLE_PRIORITY + 4UL )
#define tHIGHEST (tskIDLE_PRIORITY + 5UL )
...
void vLowTask(void *pvParameters); //led toggle function prototype
void vMedTask(void *pvParameters); //led toggle function prototype
void vHighTask(void *pvParameters); //led toggle function prototype
...
//Create sems according to priority
	SemaphoreHandle_t xSemaphoreHighP = NULL;
	SemaphoreHandle_t xSemaphoreMedP = NULL;
	SemaphoreHandle_t xPCPsLow = NULL;
	SemaphoreHandle_t xPCPsHigh1 = NULL;
	SemaphoreHandle_t xPCPsHigh2 = NULL;
/*-----------------------------------------------------------*/

int main(void)
{
	//put sem handler pointers in array to be used by low task to release semaphores
	SemaphoreHandle_t *ptr[5];

	/* Configure the hardware ready to run the test. */
	prvSetupHardware();
	printf("start run\n");
	xSemaphoreHighP = xSemaphoreCreateBinary();
	xSemaphoreMedP = xSemaphoreCreateBinary();
	printf("\nbinaries created");
	xPCPsLow = (SemaphoreHandle_t) xSemaphoreCreatePCP(tskIDLE_PRIORITY + 2UL);
	xPCPsHigh1 = (SemaphoreHandle_t) xSemaphoreCreatePCP(tskIDLE_PRIORITY + 3UL);
	xPCPsHigh2 = (SemaphoreHandle_t) xSemaphoreCreatePCP(tskIDLE_PRIORITY + 3UL);
	printf("\nPCP created");

	if( ( xSemaphoreHighP != NULL ) && ( xSemaphoreMedP != NULL) )
	{
		// The semaphore was created successfully.
		// The semaphore can now be used.
		printf("\nSems created");
		//xSemaphoreTake( xSemaphoreHighP, portMAX_DELAY );
		//xSemaphoreTake( xSemaphoreMedP, portMAX_DELAY );
		//printf("\nSems can only be given since xSemaphoreCreateBinary was used (not vSemaphoreCreateBinary_");

		ptr[0] = xSemaphoreHighP;
		ptr[1] = xSemaphoreMedP;
		ptr[2] = xPCPsLow;
		ptr[3] = xPCPsHigh1;
		ptr[4] = xPCPsHigh2;

		xTaskCreate( vLowTask, (const signed char*)"LowTask",
			configMINIMAL_STACK_SIZE, ptr, tLOW, NULL );
		xTaskCreate( vMedTask, (const signed char*)"MedTask",
			configMINIMAL_STACK_SIZE, ptr, tMED, NULL );
		xTaskCreate( vHighTask, (const signed char*)"HighTask",
			configMINIMAL_STACK_SIZE, ptr, tHIGH, NULL );
	...
	{
		printf("\nSemaphore creation failed. Demo terminated.");
	}
...
void vLowTask(void *pvParameters)
{
	int i;
	int j = 0;
	SemaphoreHandle_t xSemaphoreMed;
	SemaphoreHandle_t xPCPlow;
	SemaphoreHandle_t xSemaphoreHigh;
	SemaphoreHandle_t xPCPhigh;

	xSemaphoreHigh = xSemaphoreHighP;
	xSemaphoreMed = xSemaphoreMedP;
	xPCPhigh = xPCPsHigh1;
	xPCPlow = xPCPsLow;

	//loop to represent non critical section
	//0x1036640 represents roughly one second of execution time
	for (i = 0; i < 0x1036640; i++){
				j++;
	}

	//acquire PCP lock
	xSemaphoreTakePCP(xPCPlow);
	printf("\nlow takes pcplow");
	xSemaphoreGive(xSemaphoreMed);
	xSemaphoreTakePCP(xPCPhigh);
	printf("\nlow takes pcphigh");
	xSemaphoreGive(xSemaphoreHigh);
	printf("\nlow dropping pcphigh");
	xSemaphoreGivePCP(xPCPhigh);
	printf("\nlow drops pcphigh");
	xSemaphoreGivePCP(xPCPlow);
	printf("\nlow drops pcplow. low exiting");
	vTaskDelete( NULL );


}

void vMedTask(void *pvParameters)
{
//	-block on semaphore
	SemaphoreHandle_t xSemaphoreMed;
	SemaphoreHandle_t xPCP;
	xSemaphoreMed = xSemaphoreMedP;
	xPCP = xPCPsLow;

	xSemaphoreTake(xSemaphoreMed, portMAX_DELAY);
	printf("\nmedium unblocked & start");
//	-try to acquire pcp
	printf("\nmed tries to take lowpcp");
	xSemaphoreTakePCP(xPCP);
	printf("\nmed takes low pcp");
	xSemaphoreGivePCP(xPCP);
	printf("\nmed drops low pcp. medium exiting.");
	vTaskDelete( NULL );
}
...
{
	SemaphoreHandle_t xSemaphoreHigh;
	SemaphoreHandle_t xPCP1;
	SemaphoreHandle_t xPCP2;
	xSemaphoreHigh = xSemaphoreHighP;
	xPCP1 = xPCPsHigh2;
	xPCP2 = xPCPsHigh1;

	//block on initial semaphore
	xSemaphoreTake(xSemaphoreHigh, portMAX_DELAY);
	printf("\nhigh unblocked & start");
	//try to acquire pcp
	printf("\nhigh tries to take high pcp");
	xSemaphoreTakePCP(xPCP1);
	printf("\nhigh takes high pcp");
	xSemaphoreGivePCP(xPCP1);
	printf("\nhigh gives high pcp");
	printf("\nhigh tries to take 2nd high pcp");
	xSemaphoreTakePCP(xPCP2);
	printf("\nhigh high takes 2nd high pcp");
	xSemaphoreGivePCP(xPCP2);
	printf("\nhigh gives second high pcp. exiting");
	vTaskDelete( NULL );
}
/*-----------------------------------------------------------*/
//functions section
...
long lHigherPriorityTaskWoken = pdFALSE;
	...
	/* Only line 0 is enabled, so there is no need to test which line generated
	the interrupt. */
